...
// ---------------------------------------------------------------------------
// YouTube oEmbed proxy — free, keyless fallback for video metadata when the
// youtube/get-video-details integration is unavailable. Server-side fetch
// avoids the browser CORS restriction on YouTube's oEmbed endpoint.
// ---------------------------------------------------------------------------

app.get('/api/yt-oembed', async (c) => {
  ...
  try {
    ...
    if (!res.ok) return c.json({ error: 'oembed unavailable' }, 502)
    ...
    return c.json({ error: 'oembed fetch failed' }, 502)
  }
})
